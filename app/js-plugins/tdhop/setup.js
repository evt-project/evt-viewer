...
* This file handle functions needed to set up 3DHOP scene.
* @author FS
*/
var start=0;
var name="";
...
//-- HOTSPOTS --
//-- Convert TO Global/Local SPOTMAKER, turn the coordinates from local to global when clicking on the hotspot. */
...
function convertToLocal(state) {
   ...
   if ((state[5] * presenter.sceneRadiusInv - 0.9) == 1.10 ) {
      ...
      // manage the zoom with the settings (compared to SPOTMAKER settings)
      newstate[5] = (state[5] - (0.9 / presenter.sceneRadiusInv)) * presenter.sceneRadiusInv;
      if (newstate[5] < 0) {
         newstate[5] *= -1;
      }
   }
   //newstate[5] = state[5] * presenter.sceneRadiusInv;/*start;/*state[5] * presenter.sceneRadiusInv-0.9;*/
   return newstate;
}
//-- PAGE RESIZE
