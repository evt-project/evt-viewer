...
.service('evtProjectInfoParser', function($q, parsedData, evtParser, xmlParser) {
     ...
     * @author FS
     */
    parser.parselistObjectription = function (teiHeader) {
      var currentDocument = angular.element(teiHeader);
      angular.forEach(currentDocument.find(listObject.replace(/[<>]/g, '')),
          function(element) {
              var listObjectContent = evtParser.parseXMLElement(teiHeader, element, { skip: skipElementsFromParser, exclude: skipElementsFromInfo, context:'projectInfo' }).outerHTML;
